...
import { runAgentWithWorkflow } from "./run-workflow.js";
import type {
  RunFailureRecord,
  RunAgentResult,
  RunTaskInput,
...
export function buildRunTaskWorkflowPlanningEvent(params: {
...
export function buildTokenUsageArtifactAttempt(params: {
  ...
  const workflowNodeByStep = new Map(
    params.mainLoopEntries.map((entry) => [
      entry.step,
      {
        id: entry.workflow_node_id,
        kind: entry.workflow_node_kind,
      },
    ]),
  );
  ...
  const stageRows = params.modelInvocations.map((trace, index) => {
    ...
    const workflowNodeId =
      typeof trace.meta?.workflowNodeId === "string"
  ...
  for (const stepUsage of params.stepTokenUsage) {
    ...
    const workflowNode = workflowNodeByStep.get(stepUsage.step);
    ordered.push({
      sequence: 0,
      kind: "executor_step",
      phase: "executor",
      ...
      step: stepUsage.step,
      ...(stepKind ? { stepKind } : {}),
      ...(workflowNode?.id ? { workflowNodeId: workflowNode.id } : {}),
      ...(workflowNode?.kind ? { workflowNodeKind: workflowNode.kind } : {}),
...
export async function runTask(input: RunTaskInput): Promise<RunTaskResult> {
  ...
  const failedRuns = await __runTaskRetryLoopForTests({
    ...
    executeRun: async (runIndex, attemptOrdinal) => {
      ...
      try {
        runAgentResult = await runAgentWithWorkflow(
          {
            session: {
          ...
          input.workflowMaxParallelNodes ?? 4,
        );
      } catch (error) {
      ...
      const jsonlEntry = {
        ...
        ...(runAgentResult.successVerification
          ...
          : {}),
        ...(runAgentResult.workflow
          ? { workflow: runAgentResult.workflow }
          : {}),
