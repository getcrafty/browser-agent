...
import { formatWorkflowPlanningEventBlock } from "./workflow-logging.js";
import {
  resetOrchestrationLogsDir,
  saveOrchestrationLogs,
} from "./orchestration-logging.js";
...
export async function main(
  ...
  resetOrchestrationLogsDir(
    config.featureFlags.workflowOrchestration,
    orchestrationLogsDir,
  );
  ...
  console.log(
    `Stage LLMs: ${JSON.stringify({
      workflowPlanner: {
        provider: stageOptions.workflowPlanner.provider,
      ...
      dismissCookieBanner: {
        ...
        reasoningEffort: stageOptions.dismissCookieBanner.reasoningEffort,
      },
      createPlan: {
      ...
      createChecklist: {
        provider: (stageOptions.createChecklist ?? stageOptions.createPlan)
          .provider,
        model: (stageOptions.createChecklist ?? stageOptions.createPlan).model,
        reasoningEffort: (
      ...
      preExecutionDomPruning: {
        ...
        reasoningEffort: stageOptions.preExecutionDomPruning.reasoningEffort,
      },
      runAgent: {
  ...
  if (config.featureFlags.workflowOrchestration) {
    console.log(
      `Workflow orchestration enabled (max ${workflowMaxParallelNodes} parallel nodes).`,
    );
  }
  console.log(
  ...
  if (config.featureFlags.workflowOrchestration) {
    console.log(`Orchestration logs directory: ${orchestrationLogsDir}`);
  }
  console.log(
            ...
            async () =>
              runTaskFn({
                ...
                onWorkflowPlanned: (event) => {
                  ...
                  const block = formatWorkflowPlanningEventBlock(event, {
                    taskNumber: configIndex,
                    totalTasks: configuredTasks.length,
                  });
