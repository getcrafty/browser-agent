...
async function getFormControlBackendNodeIdsInSubtree(
  ...
  const appendIfFormControl = async (nodeId: number): Promise<void> => {
    ...
    const nodeName =
      typeof node.nodeName === "string" ? node.nodeName.toUpperCase() : "";
...
async function getLiveFormControlValuesInSubtree(
  ...
  const backendIds = await getFormControlBackendNodeIdsInSubtree(b, rootNodeId);
...
async function extractIsolatedRenderPayload(
  ...
  if (
    ...
    throw new Error("Failed to extract element HTML/CSS for isolated render");
  }
  return payload;
}
...
<html>
  ...
  <body>${payload.rootHtml}</body>
    ...
    try {
      ...
      // The target may already have disappeared from the scope.
    }
    try {
...
export async function screenshotElementInIsolatedPage(
  ...
  for (const candidateBid of candidates) {
    try {
      ...
      const liveFormControlValues = await getLiveFormControlValuesInSubtree(
        b,
        nodeId,
      );
